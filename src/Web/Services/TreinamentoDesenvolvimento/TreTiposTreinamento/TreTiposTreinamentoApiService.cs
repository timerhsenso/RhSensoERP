...
/// <summary>
/// Serviço de API para Cadastro de Tipo de Treinamento.
...
/// </summary>
public class TreTiposTreinamentoApiService 
    : BaseApiService<TreTiposTreinamentoDto, CreateTreTiposTreinamentoRequest, UpdateTreTiposTreinamentoRequest, int>,
      ITreTiposTreinamentoApiService
{
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[TRETIPOSTREINAMENTO] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
    ...
    public async Task ToggleAtivoAsync(int id, bool ativo, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "[TRETIPOSTREINAMENTO] PATCH {Route}/{Id}/toggle-ativo - Body: {Body}",
                _baseEndpoint,
                id,
                json);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "[TRETIPOSTREINAMENTO] Erro ao alternar Ativo - Status: {Status}, Content: {Content}",
                    response.StatusCode,
                    errorContent);
            ...
            _logger.LogInformation(
                "[TRETIPOSTREINAMENTO] Status Ativo alterado para {Ativo} - ID: {Id}",
                ativo,
                id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TRETIPOSTREINAMENTO] Erro de conexão em ToggleAtivoAsync");
            throw new InvalidOperationException("Erro de conexão com o servidor", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TRETIPOSTREINAMENTO] Exceção em ToggleAtivoAsync");
            throw;
        }
    }
