...
/// <summary>
/// Implementação do serviço de API para Vínculo Empregatício.
...
/// </remarks>
public class Tvin1ApiService : ITvin1ApiService
{
    private readonly HttpClient _httpClient;
    private readonly IHttpContextAccessor _httpContextAccessor;
    private readonly ILogger<Tvin1ApiService> _logger;
    private readonly JsonSerializerOptions _jsonOptions;

    // =========================================================================
    // v3.2 - ROTA API DO MANIFESTO (não mais construída automaticamente)
    // =========================================================================
    private const string ApiRoute = "api/administracaopessoal/vinculoempregaticio";

    public Tvin1ApiService(
        HttpClient httpClient,
        IHttpContextAccessor httpContextAccessor,
        ILogger<Tvin1ApiService> logger)
    {
    ...
    private async Task SetAuthHeaderAsync()
    {
        ...
        if (context?.User?.Identity?.IsAuthenticated == true)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                _logger.LogDebug("[TVIN1] Token JWT configurado");
            }
            else
            {
                _logger.LogWarning("[TVIN1] Token JWT não encontrado");
            }
        }
        else
        {
            _logger.LogWarning("[TVIN1] Usuário não autenticado");
        }
    }
        ...
        string operation)
    {
        ...
        _logger.LogDebug("[TVIN1] {Op} - Status: {Status}, Content: {Content}", 
    ...
    public async Task<ApiResponse<IEnumerable<Tvin1Dto>>> GetAllAsync()
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<IEnumerable<Tvin1Dto>>(response, "GetAll");

            var content = await response.Content.ReadAsStringAsync();
            var backendResult = JsonSerializer.Deserialize<BackendResult<List<Tvin1Dto>>>(content, _jsonOptions);
            
            if (backendResult?.IsSuccess == true)
            {
                IEnumerable<Tvin1Dto> items = backendResult.Value ?? backendResult.Data ?? new List<Tvin1Dto>();
                return Success(items);
            }

            return Fail<IEnumerable<Tvin1Dto>>(backendResult?.Error?.Message ?? "Erro ao buscar registros");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TVIN1] Erro de conexão em GetAllAsync");
            return Fail<IEnumerable<Tvin1Dto>>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TVIN1] Exceção em GetAllAsync");
            return Fail<IEnumerable<Tvin1Dto>>(ex.Message);
        }
    }

    public async Task<ApiResponse<PagedResult<Tvin1Dto>>> GetPagedAsync(int page, int pageSize, string? search = null)
    {
        try
        {
            await SetAuthHeaderAsync();
            ...
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<PagedResult<Tvin1Dto>>(response, "GetPaged");

            var content = await response.Content.ReadAsStringAsync();
            var backendResult = JsonSerializer.Deserialize<BackendResult<PagedResult<Tvin1Dto>>>(content, _jsonOptions);
            ...
            return Fail<PagedResult<Tvin1Dto>>(backendResult?.Error?.Message ?? "Erro ao buscar registros paginados");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TVIN1] Erro de conexão em GetPagedAsync");
            return Fail<PagedResult<Tvin1Dto>>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TVIN1] Exceção em GetPagedAsync");
            return Fail<PagedResult<Tvin1Dto>>(ex.Message);
        }
    }

    public async Task<ApiResponse<Tvin1Dto>> GetByIdAsync(Guid id)
    {
        try
        {
            await SetAuthHeaderAsync();
            ...
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<Tvin1Dto>(response, "GetById");

            var content = await response.Content.ReadAsStringAsync();
            var backendResult = JsonSerializer.Deserialize<BackendResult<Tvin1Dto>>(content, _jsonOptions);
            ...
            return Fail<Tvin1Dto>(backendResult?.Error?.Message ?? "Registro não encontrado");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TVIN1] Erro de conexão em GetByIdAsync");
            return Fail<Tvin1Dto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TVIN1] Exceção em GetByIdAsync");
            return Fail<Tvin1Dto>(ex.Message);
        }
    }

    public async Task<ApiResponse<Tvin1Dto>> CreateAsync(CreateTvin1Request request)
    {
        try
        {
            await SetAuthHeaderAsync();
            ...
            _logger.LogDebug("[TVIN1] POST {Route} - Body: {Body}", ApiRoute, json);
            
            var response = await _httpClient.PostAsync(ApiRoute, content);
            
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<Tvin1Dto>(response, "Create");
            ...
            return Fail<Tvin1Dto>(createResult?.Error?.Message ?? "Erro ao criar registro");
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TVIN1] Erro de conexão em CreateAsync");
            return Fail<Tvin1Dto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TVIN1] Exceção em CreateAsync");
            return Fail<Tvin1Dto>(ex.Message);
        }
    }

    public async Task<ApiResponse<Tvin1Dto>> UpdateAsync(Guid id, UpdateTvin1Request request)
    {
        try
        {
            await SetAuthHeaderAsync();
            ...
            _logger.LogDebug("[TVIN1] PUT {Route}/{Id} - Body: {Body}", ApiRoute, id, json);
            
            var response = await _httpClient.PutAsync($"{ApiRoute}/{id}", content);
            
            if (!response.IsSuccessStatusCode)
                return await ProcessResponseAsync<Tvin1Dto>(response, "Update");

            return await GetByIdAsync(id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[TVIN1] Erro de conexão em UpdateAsync");
            return Fail<Tvin1Dto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TVIN1] Exceção em UpdateAsync");
            return Fail<Tvin1Dto>(ex.Message);
        }
    }
    ...
    public async Task<ApiResponse<bool>> DeleteAsync(Guid id)
    {
        ...
        {
            _logger.LogError(ex, "[TVIN1] Erro de conexão em DeleteAsync");
            return Fail<bool>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TVIN1] Exceção em DeleteAsync");
    ...
    public async Task<ApiResponse<bool>> DeleteMultipleAsync(IEnumerable<Guid> ids)
    {
        ...
        {
            _logger.LogError(ex, "[TVIN1] Exceção em DeleteMultipleAsync");
            return Fail<bool>(ex.Message);
        }
    }
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<Guid> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[TVIN1] DELETE {Route}/batch - Body: {Body}", ApiRoute, json);
        ...
        {
            _logger.LogError(ex, "[TVIN1] Erro de conexão em DeleteBatchAsync");
            return Fail<BatchDeleteResultDto>("Erro de conexão com o servidor");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[TVIN1] Exceção em DeleteBatchAsync");
