...
// ApiRoute: api/gestaoterceirosprestadores/capbloqueiospessoa
...
/// <summary>
/// Serviço de API para CapBloqueiosPessoa.
...
/// </summary>
public class CapBloqueiosPessoaApiService 
    : BaseApiService<CapBloqueiosPessoaDto, CreateCapBloqueiosPessoaRequest, UpdateCapBloqueiosPessoaRequest, int>,
      ICapBloqueiosPessoaApiService
{
    private const string ApiRoute = "api/gestaoterceirosprestadores/capbloqueiospessoa";
    ...
    // =========================================================================
    public CapBloqueiosPessoaApiService(
        HttpClient httpClient,
        ILogger<CapBloqueiosPessoaApiService> logger,
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[CAPBLOQUEIOSPESSOA] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
        ...
        {
            _logger.LogError(ex, "[CAPBLOQUEIOSPESSOA] Erro de conexão em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
        ...
        {
            _logger.LogError(ex, "[CAPBLOQUEIOSPESSOA] Exceção em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
    ...
    public async Task ToggleAtivoAsync(int id, bool ativo, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "[CAPBLOQUEIOSPESSOA] PATCH {Route}/{Id}/toggle-ativo - Body: {Body}",
                _baseEndpoint,
                id,
                json);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "[CAPBLOQUEIOSPESSOA] Erro ao alternar Ativo - Status: {Status}, Content: {Content}",
                    response.StatusCode,
                    errorContent);
            ...
            _logger.LogInformation(
                "[CAPBLOQUEIOSPESSOA] Status Ativo alterado para {Ativo} - ID: {Id}",
                ativo,
                id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[CAPBLOQUEIOSPESSOA] Erro de conexão em ToggleAtivoAsync");
            throw new InvalidOperationException("Erro de conexão com o servidor", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[CAPBLOQUEIOSPESSOA] Exceção em ToggleAtivoAsync");
            throw;
        }
    }
