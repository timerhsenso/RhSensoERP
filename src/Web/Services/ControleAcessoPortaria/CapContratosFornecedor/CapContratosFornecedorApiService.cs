...
/// <summary>
/// Serviço de API para CapContratosFornecedor.
...
/// </summary>
public class CapContratosFornecedorApiService 
    : BaseApiService<CapContratosFornecedorDto, CreateCapContratosFornecedorRequest, UpdateCapContratosFornecedorRequest, int>,
      ICapContratosFornecedorApiService
{
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[CAPCONTRATOSFORNECEDOR] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
    ...
    public async Task ToggleAtivoAsync(int id, bool ativo, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "[CAPCONTRATOSFORNECEDOR] PATCH {Route}/{Id}/toggle-ativo - Body: {Body}",
                _baseEndpoint,
                id,
                json);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "[CAPCONTRATOSFORNECEDOR] Erro ao alternar Ativo - Status: {Status}, Content: {Content}",
                    response.StatusCode,
                    errorContent);
            ...
            _logger.LogInformation(
                "[CAPCONTRATOSFORNECEDOR] Status Ativo alterado para {Ativo} - ID: {Id}",
                ativo,
                id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[CAPCONTRATOSFORNECEDOR] Erro de conexão em ToggleAtivoAsync");
            throw new InvalidOperationException("Erro de conexão com o servidor", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[CAPCONTRATOSFORNECEDOR] Exceção em ToggleAtivoAsync");
            throw;
        }
    }
