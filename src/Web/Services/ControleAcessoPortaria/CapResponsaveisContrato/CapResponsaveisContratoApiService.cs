...
/// <summary>
/// Serviço de API para CapResponsaveisContrato.
...
/// </summary>
public class CapResponsaveisContratoApiService 
    : BaseApiService<CapResponsaveisContratoDto, CreateCapResponsaveisContratoRequest, UpdateCapResponsaveisContratoRequest, int>,
      ICapResponsaveisContratoApiService
{
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[CAPRESPONSAVEISCONTRATO] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
    ...
    public async Task ToggleAtivoAsync(int id, bool ativo, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "[CAPRESPONSAVEISCONTRATO] PATCH {Route}/{Id}/toggle-ativo - Body: {Body}",
                _baseEndpoint,
                id,
                json);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "[CAPRESPONSAVEISCONTRATO] Erro ao alternar Ativo - Status: {Status}, Content: {Content}",
                    response.StatusCode,
                    errorContent);
            ...
            _logger.LogInformation(
                "[CAPRESPONSAVEISCONTRATO] Status Ativo alterado para {Ativo} - ID: {Id}",
                ativo,
                id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[CAPRESPONSAVEISCONTRATO] Erro de conexão em ToggleAtivoAsync");
            throw new InvalidOperationException("Erro de conexão com o servidor", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[CAPRESPONSAVEISCONTRATO] Exceção em ToggleAtivoAsync");
            throw;
        }
    }
