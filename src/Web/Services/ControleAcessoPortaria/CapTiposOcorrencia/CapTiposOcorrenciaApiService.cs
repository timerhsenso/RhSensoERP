...
/// <summary>
/// Serviço de API para Cadastro de Tipo de Ocorrência.
...
/// </summary>
public class CapTiposOcorrenciaApiService 
    : BaseApiService<CapTiposOcorrenciaDto, CreateCapTiposOcorrenciaRequest, UpdateCapTiposOcorrenciaRequest, int>,
      ICapTiposOcorrenciaApiService
{
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[CAPTIPOSOCORRENCIA] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
    ...
    public async Task ToggleAtivoAsync(int id, bool ativo, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "[CAPTIPOSOCORRENCIA] PATCH {Route}/{Id}/toggle-ativo - Body: {Body}",
                _baseEndpoint,
                id,
                json);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "[CAPTIPOSOCORRENCIA] Erro ao alternar Ativo - Status: {Status}, Content: {Content}",
                    response.StatusCode,
                    errorContent);
            ...
            _logger.LogInformation(
                "[CAPTIPOSOCORRENCIA] Status Ativo alterado para {Ativo} - ID: {Id}",
                ativo,
                id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[CAPTIPOSOCORRENCIA] Erro de conexão em ToggleAtivoAsync");
            throw new InvalidOperationException("Erro de conexão com o servidor", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[CAPTIPOSOCORRENCIA] Exceção em ToggleAtivoAsync");
            throw;
        }
    }
