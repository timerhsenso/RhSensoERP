...
/// <summary>
/// Serviço de API para CapContatosEmergencia.
...
/// </summary>
public class CapContatosEmergenciaApiService 
    : BaseApiService<CapContatosEmergenciaDto, CreateCapContatosEmergenciaRequest, UpdateCapContatosEmergenciaRequest, int>,
      ICapContatosEmergenciaApiService
{
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[CAPCONTATOSEMERGENCIA] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
    ...
    public async Task ToggleAtivoAsync(int id, bool ativo, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "[CAPCONTATOSEMERGENCIA] PATCH {Route}/{Id}/toggle-ativo - Body: {Body}",
                _baseEndpoint,
                id,
                json);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "[CAPCONTATOSEMERGENCIA] Erro ao alternar Ativo - Status: {Status}, Content: {Content}",
                    response.StatusCode,
                    errorContent);
            ...
            _logger.LogInformation(
                "[CAPCONTATOSEMERGENCIA] Status Ativo alterado para {Ativo} - ID: {Id}",
                ativo,
                id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[CAPCONTATOSEMERGENCIA] Erro de conexão em ToggleAtivoAsync");
            throw new InvalidOperationException("Erro de conexão com o servidor", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[CAPCONTATOSEMERGENCIA] Exceção em ToggleAtivoAsync");
            throw;
        }
    }
