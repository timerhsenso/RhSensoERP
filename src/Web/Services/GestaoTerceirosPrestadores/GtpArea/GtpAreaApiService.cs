...
/// </summary>
public class GtpAreaApiService 
    : BaseApiService<GtpAreaDto, CreateGtpAreaRequest, UpdateGtpAreaRequest, int>,
      IGtpAreaApiService
{
    ...
    // CONSTRUTOR
    // =========================================================================
    public GtpAreaApiService(
        HttpClient httpClient,
        ILogger<GtpAreaApiService> logger,
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[GTPAREA] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
        ...
        {
            _logger.LogError(ex, "[GTPAREA] Erro de conexão em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
        ...
        {
            _logger.LogError(ex, "[GTPAREA] Exceção em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
    ...
    public async Task ToggleAtivoAsync(int id, bool ativo, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "[GTPAREA] PATCH {Route}/{Id}/toggle-ativo - Body: {Body}",
                _baseEndpoint,
                id,
                json);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "[GTPAREA] Erro ao alternar Ativo - Status: {Status}, Content: {Content}",
                    response.StatusCode,
                    errorContent);
            ...
            _logger.LogInformation(
                "[GTPAREA] Status Ativo alterado para {Ativo} - ID: {Id}",
                ativo,
                id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[GTPAREA] Erro de conexão em ToggleAtivoAsync");
            throw new InvalidOperationException("Erro de conexão com o servidor", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[GTPAREA] Exceção em ToggleAtivoAsync");
            throw;
        }
    }
