...
/// <summary>
/// Serviço de API para Cadastro de Terceiros teste.
...
/// </summary>
public class GTP_AreaApiService 
    : BaseApiService<GTP_AreaDto, CreateGTP_AreaRequest, UpdateGTP_AreaRequest, int>,
      IGTP_AreaApiService
{
    private const string ApiRoute = "api/gestaoterceirosprestadores/gtparea";
    ...
    // CONSTRUTOR
    // =========================================================================
    public GTP_AreaApiService(
        HttpClient httpClient,
        ILogger<GTP_AreaApiService> logger,
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<int> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[GTP_AREA] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
        ...
        {
            _logger.LogError(ex, "[GTP_AREA] Erro de conexão em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
        ...
        {
            _logger.LogError(ex, "[GTP_AREA] Exceção em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
    ...
    public async Task ToggleAtivoAsync(int id, bool ativo, CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "[GTP_AREA] PATCH {Route}/{Id}/toggle-ativo - Body: {Body}",
                _baseEndpoint,
                id,
                json);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "[GTP_AREA] Erro ao alternar Ativo - Status: {Status}, Content: {Content}",
                    response.StatusCode,
                    errorContent);
            ...
            _logger.LogInformation(
                "[GTP_AREA] Status Ativo alterado para {Ativo} - ID: {Id}",
                ativo,
                id);
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "[GTP_AREA] Erro de conexão em ToggleAtivoAsync");
            throw new InvalidOperationException("Erro de conexão com o servidor", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[GTP_AREA] Exceção em ToggleAtivoAsync");
            throw;
        }
    }
