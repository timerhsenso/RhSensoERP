...
// ApiRoute: api/tabelascompartilhadas/tabelaauxiliar
...
/// <summary>
/// Serviço de API para Tabela Auxiliar.
...
/// </summary>
public class TabelaAuxiliarApiService 
    : BaseApiService<TabelaAuxiliarDto, CreateTabelaAuxiliarRequest, UpdateTabelaAuxiliarRequest, string>,
      ITabelaAuxiliarApiService
{
    private const string ApiRoute = "api/tabelascompartilhadas/tabelaauxiliar";
    ...
    // CONSTRUTOR
    // =========================================================================
    public TabelaAuxiliarApiService(
        HttpClient httpClient,
        ILogger<TabelaAuxiliarApiService> logger,
    ...
    /// </summary>
    public override async Task<ApiResponse<TabelaAuxiliarDto>> GetByIdAsync(string id)
    {
        try
        {
            ...
            _logger.LogDebug("🔍 [TABELAAUXILIAR] GetByIdAsync - Key: {Id} → URL: {Url}", id, url);
            ...
            {
                var result = JsonSerializer.Deserialize<ApiResponse<TabelaAuxiliarDto>>(content, _jsonOptions);
                return result ?? new ApiResponse<TabelaAuxiliarDto>
                {
                    Success = false,
            ...
            _logger.LogWarning("⚠️ [TABELAAUXILIAR] GetByIdAsync - ID: {Id}, Status: {Status}, Body: {Body}",
                id, response.StatusCode, content);
            return new ApiResponse<TabelaAuxiliarDto>
            {
                Success = false,
        ...
        {
            _logger.LogError(ex, "❌ [TABELAAUXILIAR] Erro GetByIdAsync {Id}", id);
            return new ApiResponse<TabelaAuxiliarDto>
            {
                Success = false,
                Error = new ApiError { Message = "Erro ao buscar registro" }
    ...
    /// </summary>
    public override async Task<ApiResponse<TabelaAuxiliarDto>> UpdateAsync(string id, UpdateTabelaAuxiliarRequest dto)
    {
        try
        {
            await AddAuthorizationHeaderAsync();
            ...
            _logger.LogWarning("✏️ [TABELAAUXILIAR] UpdateAsync - ID: {Id} → URL: {Url} | Body: {Body}", id, url, json);

            var response = await _httpClient.PutAsync(url, httpContent);
            var content = await response.Content.ReadAsStringAsync();

            // ⭐ v6.3: Loga response raw
            _logger.LogDebug("✏️ [TABELAAUXILIAR] UpdateAsync - Status: {Status} | Response: {Body}",
            ...
            _logger.LogWarning("⚠️ [TABELAAUXILIAR] UpdateAsync FALHOU - ID: {Id}, Status: {Status}, Response: {Body}",
                id, response.StatusCode, content);
        ...
        {
            _logger.LogError(ex, "❌ [TABELAAUXILIAR] Erro UpdateAsync {Id}", id);
            return new ApiResponse<TabelaAuxiliarDto>
            {
                Success = false,
    ...
    public override async Task<ApiResponse<bool>> DeleteAsync(string id)
    {
        try
        {
            ...
            var path = CompositeKeyToPath(id);
            var url = $"{_baseEndpoint}/{path}";
            _logger.LogDebug("🗑️ [TABELAAUXILIAR] DeleteAsync - Key: {Id} → URL: {Url}", id, url);
            ...
            if (response.IsSuccessStatusCode)
            {
                _logger.LogInformation("✅ [TABELAAUXILIAR] Registro {Id} excluído", id);
                return new ApiResponse<bool> { Success = true, Data = true };
            }

            _logger.LogWarning("⚠️ [TABELAAUXILIAR] DeleteAsync - ID: {Id}, Status: {Status}, Body: {Body}",
                id, response.StatusCode, content);
        ...
        {
            _logger.LogError(ex, "❌ [TABELAAUXILIAR] Erro DeleteAsync {Id}", id);
            return new ApiResponse<bool>
            {
                Success = false,
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<string> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[TABELAAUXILIAR] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
        ...
        {
            _logger.LogError(ex, "[TABELAAUXILIAR] Erro de conexão em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
        ...
        {
            _logger.LogError(ex, "[TABELAAUXILIAR] Exceção em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
