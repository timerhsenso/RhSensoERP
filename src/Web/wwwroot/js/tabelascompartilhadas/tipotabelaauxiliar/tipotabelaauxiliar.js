 ...
 * ============================================================================
 * Arquivo: wwwroot/js/tabelascompartilhadas/tipotabelaauxiliar/tipotabelaauxiliar.js
...
class TipoTabelaAuxiliarCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        console.log(enable 
            ? '✏️ [TipoTabelaAuxiliar] Campos PK habilitados para edição (criação)'
            : '🔒 [TipoTabelaAuxiliar] Campos PK desabilitados (edição)');
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [TipoTabelaAuxiliar] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        console.log('📤 [TipoTabelaAuxiliar] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [TipoTabelaAuxiliar] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [TipoTabelaAuxiliar] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {{
        ...
        id = String(id).trim();
        if (!id) {{
            console.warn('⚠️ [TipoTabelaAuxiliar] ID vazio para row:', row);
        }}
        return id;
    }}
    ...
    const columns = [
        ...
        // Código de Tp Tabela (Order: 0)
        {
            data: 'cdTpTabela',
            name: 'CdTpTabela',
            title: 'Código de Tp Tabela',
            orderable: true,
            render: function (data, type, row) {
                return data !== undefined && data !== null ? data : '';
            }
        },
        // Descrição de Tabela (Order: 1)
        {
            data: 'dcTabela',
            name: 'DcTabela',
            title: 'Descrição de Tabela',
            orderable: true,
            render: function (data, type, row) {
                return data !== undefined && data !== null ? data : '';
            }
    ...
    const crud = new TipoTabelaAuxiliarCrud({
        controllerName: 'TipoTabelaAuxiliar',
        apiRoute: '/api/tabelascompartilhadas/tipotabelaauxiliar',
    ...
    console.log('✅ [TipoTabelaAuxiliar] JavaScript inicializado com sucesso!');
});
