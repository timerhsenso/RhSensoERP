/**
 * ============================================================================
 * HABILITAÇÃO DE GRUPO - JavaScript com Fix PK Texto
 * ============================================================================
 * Arquivo: wwwroot/js/seguranca/habilitacaogrupo/habilitacaogrupo.js
 * Módulo: Seguranca
 ...
 * 
 * Implementação específica do CRUD de Habilitação de Grupo.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class HabilitacaoGrupoCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        console.log(enable 
            ? '✏️ [HabilitacaoGrupo] Campos PK habilitados para edição (criação)'
            : '🔒 [HabilitacaoGrupo] Campos PK desabilitados (edição)');
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [HabilitacaoGrupo] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.CdAcoes = formData.cdAcoes || formData.CdAcoes || '';
        cleanData.CdRestric = formData.cdRestric || formData.CdRestric || '';

        console.log('📤 [HabilitacaoGrupo] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [HabilitacaoGrupo] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [HabilitacaoGrupo] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {{
        ...
        id = String(id).trim();
        if (!id) {{
            console.warn('⚠️ [HabilitacaoGrupo] ID vazio para row:', row);
        }}
        return id;
    }}
    ...
    const columns = [
        ...
        // Código de Acoes (Order: 3)
        {
            data: 'cdAcoes',
            name: 'CdAcoes',
            title: 'Código de Acoes',
            orderable: true,
            render: function (data, type, row) {
                return data !== undefined && data !== null ? data : '';
            }
        },
        // Código de Restric (Order: 4)
        {
            data: 'cdRestric',
            name: 'CdRestric',
            title: 'Código de Restric',
            orderable: true,
            render: function (data, type, row) {
                return data !== undefined && data !== null ? data : '';
            }
    ...
    const crud = new HabilitacaoGrupoCrud({
        controllerName: 'HabilitacaoGrupo',
        apiRoute: '/api/seguranca/habilitacaogrupo',
        entityName: 'Habilitação de Grupo',
        entityNamePlural: 'Habilitação de Grupos',
        idField: 'id',
    ...
    console.log('✅ [HabilitacaoGrupo] JavaScript inicializado com sucesso!');
});
