 ...
 * ============================================================================
 * Arquivo: wwwroot/js/seguranca/fucn1/fucn1.js
 * Módulo: Seguranca
...
class Fucn1Crud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [Fucn1] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [Fucn1] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [Fucn1] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        console.log('📤 [Fucn1] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [Fucn1] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [Fucn1] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [Fucn1] ID vazio para row:', row);
        }
    ...
    const crud = new Fucn1Crud({
        controllerName: 'Fucn1',
    ...
    console.log('✅ [Fucn1] JavaScript inicializado com sucesso!');
});
