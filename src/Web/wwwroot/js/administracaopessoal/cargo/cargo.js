/**
 * ============================================================================
 * CARGOS - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/administracaopessoal/cargo/cargo.js
 ...
 * 
 * Implementação específica do CRUD de Cargos.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class CargoCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [Cargo] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [Cargo] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [Cargo] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.Cdcargo = formData.cdcargo || formData.Cdcargo || '';
        cleanData.Dccargo = formData.dccargo || formData.Dccargo || '';
        cleanData.Cdtabela = formData.cdtabela || formData.Cdtabela || '';
        cleanData.Cdniveini = formData.cdniveini || formData.Cdniveini || '';
        cleanData.Cdnivefim = formData.cdnivefim || formData.Cdnivefim || '';
        cleanData.CdinStruc = formData.cdinStruc || formData.CdinStruc || '';
        cleanData.Cdcbo = formData.cdcbo || formData.Cdcbo || '';
        cleanData.Cdcbo6 = formData.cdcbo6 || formData.Cdcbo6 || '';
        cleanData.Cdgrprof = formData.cdgrprof || formData.Cdgrprof || '';


        // Integer required fields - PascalCase
        cleanData.Flativo = parseInt(formData.flativo || formData.Flativo || 0, 10);


        // DateTime fields - PascalCase
        cleanData.Dtinival = formData.dtinival || formData.Dtinival || null;
        cleanData.Dtfimval = formData.dtfimval || formData.Dtfimval || null;

        console.log('📤 [Cargo] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [Cargo] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [Cargo] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [Cargo] ID vazio para row:', row);
        }
    ...
    const columns = [
        ...
        // Data Início Validade (Order: 3)
        {
            data: 'dtinival',
            name: 'Dtinival',
            title: 'Data Início Validade',
            orderable: true,
            render: function (data, type, row) {
        ...
        // Data Fim Validade (Order: 4)
        {
            data: 'dtfimval',
            name: 'Dtfimval',
            title: 'Data Fim Validade',
            orderable: true,
            render: function (data, type, row) {
                if (type === 'display' && data) {
                    const date = new Date(data);
    ...
    const crud = new CargoCrud({
        ...
        apiRoute: '/api/administracaopessoal/cargo',
        entityName: 'Cargos',
        entityNamePlural: 'Cargoss',
        idField: 'id',
    ...
    console.log('✅ [Cargo] JavaScript inicializado com sucesso!');
});
