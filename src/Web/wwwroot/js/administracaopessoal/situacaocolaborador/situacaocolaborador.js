/**
 * ============================================================================
 * SITUAÇÃO DO COLABORADOR - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/administracaopessoal/situacaocolaborador/situacaocolaborador.js
 ...
 * 
 * Implementação específica do CRUD de Situação do Colaborador.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class SituacaoColaboradorCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [SituacaoColaborador] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [SituacaoColaborador] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [SituacaoColaborador] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.Fldemissao = formData.fldemissao || formData.Fldemissao || '';
        cleanData.FlafaStame = formData.flafaStame || formData.FlafaStame || '';
        cleanData.Cdfgts = formData.cdfgts || formData.Cdfgts || '';
        cleanData.Cdsefip = formData.cdsefip || formData.Cdsefip || '';
        cleanData.Cdsefip2 = formData.cdsefip2 || formData.Cdsefip2 || '';
        cleanData.Flpferias = formData.flpferias || formData.Flpferias || '';


        // Integer required fields - PascalCase
        cleanData.Qtdiasbene = parseInt(formData.qtdiasbene || formData.Qtdiasbene || 0, 10);
        cleanData.Qtdiasprev = parseInt(formData.qtdiasprev || formData.Qtdiasprev || 0, 10);

        console.log('📤 [SituacaoColaborador] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [SituacaoColaborador] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [SituacaoColaborador] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [SituacaoColaborador] ID vazio para row:', row);
        }
    ...
    const crud = new SituacaoColaboradorCrud({
        controllerName: 'SituacaoColaborador',
        apiRoute: '/api/administracaopessoal/situacaocolaborador',
        entityName: 'Situação do Colaborador',
        entityNamePlural: 'Situação do Colaboradors',
        idField: 'id',
    ...
    console.log('✅ [SituacaoColaborador] JavaScript inicializado com sucesso!');
});
