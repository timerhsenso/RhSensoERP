/**
 * ============================================================================
 * GRAU DE INSTRUCAO - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/administracaopessoal/grauinstrucao/grauinstrucao.js
 ...
 * 
 * Implementação específica do CRUD de Grau de instrucao.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class GrauInstrucaoCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [GrauInstrucao] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [GrauInstrucao] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [GrauInstrucao] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.DcinStruc = formData.dcinStruc || formData.DcinStruc || '';
        cleanData.Cdrais = formData.cdrais || formData.Cdrais || '';
        cleanData.Cdcaged = formData.cdcaged || formData.Cdcaged || '';
        cleanData.Cdesocial = formData.cdesocial || formData.Cdesocial || '';

        console.log('📤 [GrauInstrucao] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [GrauInstrucao] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [GrauInstrucao] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [GrauInstrucao] ID vazio para row:', row);
        }
    ...
    const crud = new GrauInstrucaoCrud({
        controllerName: 'GrauInstrucao',
        apiRoute: '/api/administracaopessoal/grauinstrucao',
        entityName: 'Grau de instrucao',
        entityNamePlural: 'Grau de instrucaos',
        idField: 'id',
    ...
    console.log('✅ [GrauInstrucao] JavaScript inicializado com sucesso!');
});
