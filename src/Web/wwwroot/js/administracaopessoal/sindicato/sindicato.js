/**
 * ============================================================================
 * SINDICATO - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/administracaopessoal/sindicato/sindicato.js
 ...
 * 
 * Implementação específica do CRUD de Sindicato.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class SindicatoCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [Sindicato] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [Sindicato] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [Sindicato] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.Cdsindicat = formData.cdsindicat || formData.Cdsindicat || '';
        cleanData.Dcsindicat = formData.dcsindicat || formData.Dcsindicat || '';
        cleanData.Dcendereco = formData.dcendereco || formData.Dcendereco || '';
        cleanData.Cgcsindicat = formData.cgcsindicat || formData.Cgcsindicat || '';
        cleanData.CdentIdade = formData.cdentIdade || formData.CdentIdade || '';
        cleanData.Cdtabbase = formData.cdtabbase || formData.Cdtabbase || '';


        // Integer required fields - PascalCase
        cleanData.Fltipo = parseInt(formData.fltipo || formData.Fltipo || 0, 10);

        console.log('📤 [Sindicato] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [Sindicato] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [Sindicato] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [Sindicato] ID vazio para row:', row);
        }
    ...
    const crud = new SindicatoCrud({
        controllerName: 'Sindicato',
        apiRoute: '/api/administracaopessoal/sindicato',
        entityName: 'Sindicato',
        entityNamePlural: 'Sindicatos',
        idField: 'id',
    ...
    console.log('✅ [Sindicato] JavaScript inicializado com sucesso!');
});
