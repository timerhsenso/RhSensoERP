/**
 * ============================================================================
 * MUNICIPIOS - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/administracaopessoal/municipio/municipio.js
 ...
 * 
 * Implementação específica do CRUD de Municipios.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class MunicipioCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [Municipio] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [Municipio] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [Municipio] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.SgeStado = formData.sgeStado || formData.SgeStado || '';
        cleanData.Nmmunicip = formData.nmmunicip || formData.Nmmunicip || '';

        console.log('📤 [Municipio] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [Municipio] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [Municipio] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [Municipio] ID vazio para row:', row);
        }
    ...
    const crud = new MunicipioCrud({
        controllerName: 'Municipio',
        apiRoute: '/api/administracaopessoal/municipio',
        entityName: 'Municipios',
        entityNamePlural: 'Municipioss',
        idField: 'id',
    ...
    console.log('✅ [Municipio] JavaScript inicializado com sucesso!');
});
