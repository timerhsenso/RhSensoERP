/**
 * ============================================================================
 * TABELA DE AGÊNCIAS - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/administracaopessoal/agenciabancaria/agenciabancaria.js
 ...
 * 
 * Implementação específica do CRUD de Tabela de Agências.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class AgenciaBancariaCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [AgenciaBancaria] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [AgenciaBancaria] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [AgenciaBancaria] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.Cdbanco = formData.cdbanco || formData.Cdbanco || '';
        cleanData.Cdagencia = formData.cdagencia || formData.Cdagencia || '';
        cleanData.Dvagencia = formData.dvagencia || formData.Dvagencia || '';
        cleanData.Nmagencia = formData.nmagencia || formData.Nmagencia || '';
        cleanData.Cdmunicip = formData.cdmunicip || formData.Cdmunicip || '';
        cleanData.Noconta = formData.noconta || formData.Noconta || '';

        console.log('📤 [AgenciaBancaria] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [AgenciaBancaria] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [AgenciaBancaria] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [AgenciaBancaria] ID vazio para row:', row);
        }
    ...
    const crud = new AgenciaBancariaCrud({
        controllerName: 'AgenciaBancaria',
        apiRoute: '/api/administracaopessoal/agenciabancaria',
        entityName: 'Tabela de Agências',
        entityNamePlural: 'Tabela de Agênciass',
        idField: 'id',
    ...
    console.log('✅ [AgenciaBancaria] JavaScript inicializado com sucesso!');
});
