/**
 * ============================================================================
 * ÁREAS - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/gestaoterceirosprestadores/gtparea/gtparea.js
 ...
 * 
 * Implementação específica do CRUD de Áreas.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class GtpAreaCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [GtpArea] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [GtpArea] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [GtpArea] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        console.log('📤 [GtpArea] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [GtpArea] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [GtpArea] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [GtpArea] ID vazio para row:', row);
        }
    ...
    const crud = new GtpAreaCrud({
        controllerName: 'GtpArea',
        apiRoute: '/api/gestaoterceirosprestadores/gtparea',
        entityName: 'Áreas',
        entityNamePlural: 'Áreass',
        idField: 'id',
    ...
    $(document).on('change', '.toggle-ativo', function () {
        ...
        console.log(`🔄 [GtpArea] Toggle Ativo - ID: ${id}, Novo valor: ${newValue}`);
        ...
        toggleDebounceTimer = setTimeout(function () {
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        console.log(`✅ [GtpArea] Toggle Ativo atualizado - ID: ${id}`);
                        ...
                        console.error(`❌ [GtpArea] Erro ao atualizar Toggle Ativo:`, response);
                ...
                error: function (xhr) {
                    // Reverte toggle em caso de erro
                    $toggle.prop('checked', currentValue);
                    console.error(`❌ [GtpArea] Erro AJAX Toggle Ativo:`, xhr);
    ...
    console.log('✅ [GtpArea] JavaScript inicializado com sucesso!');
});
