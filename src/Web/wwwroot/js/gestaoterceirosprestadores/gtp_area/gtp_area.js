/**
 * ============================================================================
 * CADASTRO DE TERCEIROS TESTE - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/gestaoterceirosprestadores/gtp_area/gtp_area.js
 ...
 * 
 * Implementação específica do CRUD de Cadastro de Terceiros teste.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class GTP_AreaCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [GTP_Area] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [GTP_Area] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [GTP_Area] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.ExigeNR = formData.exigeNR || formData.ExigeNR || '';
        cleanData.ResponsavelArea = formData.responsavelArea || formData.ResponsavelArea || '';


        // Integer required fields - PascalCase
        cleanData.IdUnidade = parseInt(formData.idUnidade || formData.IdUnidade || 0, 10);
        cleanData.GrauRisco = parseInt(formData.grauRisco || formData.GrauRisco || 0, 10);


        // Boolean fields - PascalCase
        cleanData.Ativo = formData.ativo === true || formData.Ativo === 'true' || false;

        console.log('📤 [GTP_Area] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [GTP_Area] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [GTP_Area] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [GTP_Area] ID vazio para row:', row);
        }
    ...
    const columns = [
        ...
        // Responsável (Order: 4)
        {
            data: 'responsavelArea',
            name: 'ResponsavelArea',
            title: 'Responsável',
            orderable: true,
            render: function (data, type, row) {
                return data !== undefined && data !== null ? data : '';
    ...
    const crud = new GTP_AreaCrud({
        controllerName: 'GTP_Area',
        apiRoute: '/api/gestaoterceirosprestadores/gtparea',
        entityName: 'Cadastro de Terceiros teste',
        entityNamePlural: 'Cadastro de Terceiros testes',
        idField: 'id',
    ...
    $(document).on('change', '.toggle-ativo', function () {
        ...
        console.log(`🔄 [GTP_Area] Toggle Ativo - ID: ${id}, Novo valor: ${newValue}`);
        ...
        toggleDebounceTimer = setTimeout(function () {
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        console.log(`✅ [GTP_Area] Toggle Ativo atualizado - ID: ${id}`);
                        ...
                        console.error(`❌ [GTP_Area] Erro ao atualizar Toggle Ativo:`, response);
                ...
                error: function (xhr) {
                    // Reverte toggle em caso de erro
                    $toggle.prop('checked', currentValue);
                    console.error(`❌ [GTP_Area] Erro AJAX Toggle Ativo:`, xhr);
    ...
    console.log('✅ [GTP_Area] JavaScript inicializado com sucesso!');
});
