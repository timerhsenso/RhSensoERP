/**
 * ============================================================================
 * TABELA DE CNAE - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/gestaoterceirosprestadores/shr_cnae/shr_cnae.js
 * Módulo: GestaoTerceirosPrestadores
 * Versão: 5.1 (NAVEGAÇÕES COM ORDENAÇÃO CORRETA)
 * Gerado por: GeradorFullStack v5.1
 * Data: 2026-02-25 11:14:58
 * 
 * Changelog v5.1:
 ...
 * 
 * Implementação específica do CRUD de Tabela de CNAE.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class SHR_CNAECrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [SHR_CNAE] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [SHR_CNAE] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [SHR_CNAE] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.CodigoLimpo = formData.codigoLimpo || formData.CodigoLimpo || '';
        cleanData.Descricao = formData.descricao || formData.Descricao || '';
        cleanData.Secao = formData.secao || formData.Secao || '';
        cleanData.Divisao = formData.divisao || formData.Divisao || '';
        cleanData.Grupo = formData.grupo || formData.Grupo || '';
        cleanData.Classe = formData.classe || formData.Classe || '';
        ...
        console.log('📤 [SHR_CNAE] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [SHR_CNAE] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [SHR_CNAE] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [SHR_CNAE] ID vazio para row:', row);
        }
    ...
    const columns = [
        ...
        // Versão do Registro (Order: 9)
        {
            data: 'rowVer',
            name: 'RowVer',
            title: 'Versão do Registro',
            orderable: true,
            render: function (data, type, row) {
                return data !== undefined && data !== null ? data : '';
            }
    ...
    const crud = new SHR_CNAECrud({
        controllerName: 'SHR_CNAE',
        apiRoute: '/api/gestaoterceirosprestadores/shrcnae',
        entityName: 'Tabela de CNAE',
        entityNamePlural: 'Tabela de CNAEs',
        idField: 'id',
    ...
    $(document).on('change', '.toggle-ativo', function () {
        ...
        console.log(`🔄 [SHR_CNAE] Toggle Ativo - ID: ${id}, Novo valor: ${newValue}`);
        ...
        toggleDebounceTimer = setTimeout(function () {
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        console.log(`✅ [SHR_CNAE] Toggle Ativo atualizado - ID: ${id}`);
                        ...
                        console.error(`❌ [SHR_CNAE] Erro ao atualizar Toggle Ativo:`, response);
                ...
                error: function (xhr) {
                    // Reverte toggle em caso de erro
                    $toggle.prop('checked', currentValue);
                    console.error(`❌ [SHR_CNAE] Erro AJAX Toggle Ativo:`, xhr);
    ...
    console.log('✅ [SHR_CNAE] JavaScript inicializado com sucesso!');
});
