/**
 * ============================================================================
 * TIPO DE DOCUMENTO DE FORNECEDOR - JavaScript com Fix PK Texto
 * ============================================================================
 * Arquivo: wwwroot/js/gestaoterceirosprestadores/gtp_tipodocumentofornecedor/gtp_tipodocumentofornecedor.js
 ...
 * 
 * Implementação específica do CRUD de Tipo de Documento de Fornecedor.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class GTP_TipoDocumentoFornecedorCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        console.log(enable 
            ? '✏️ [GTP_TipoDocumentoFornecedor] Campos PK habilitados para edição (criação)'
            : '🔒 [GTP_TipoDocumentoFornecedor] Campos PK desabilitados (edição)');
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [GTP_TipoDocumentoFornecedor] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.Categoria = formData.categoria || formData.Categoria || '';


        // Integer required fields - PascalCase
        cleanData.AlertarDiasAntes = parseInt(formData.alertarDiasAntes || formData.AlertarDiasAntes || 0, 10);
        cleanData.Ordem = parseInt(formData.ordem || formData.Ordem || 0, 10);


        // Boolean fields - PascalCase
        cleanData.Obrigatorio = formData.obrigatorio === true || formData.Obrigatorio === 'true' || false;
        cleanData.ControlVencimento = formData.controlVencimento === true || formData.ControlVencimento === 'true' || false;
        cleanData.Ativo = formData.ativo === true || formData.Ativo === 'true' || false;

        console.log('📤 [GTP_TipoDocumentoFornecedor] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [GTP_TipoDocumentoFornecedor] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [GTP_TipoDocumentoFornecedor] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {{
        ...
        id = String(id).trim();
        if (!id) {{
            console.warn('⚠️ [GTP_TipoDocumentoFornecedor] ID vazio para row:', row);
        }}
        return id;
    }}
    ...
    const columns = [
        ...
        // Controla Vencimento (Order: 3)
        {
            data: 'controlVencimento',
            name: 'ControlVencimento',
            title: 'Controla Vencimento',
            orderable: true,
            className: 'text-center',
            render: function (data, type, row) {
                if (type === 'display') {
        ...
        // Alertar (dias antes) (Order: 4)
        {
            data: 'alertarDiasAntes',
            name: 'AlertarDiasAntes',
            title: 'Alertar (dias antes)',
            orderable: true,
            render: function (data, type, row) {
                return data !== undefined && data !== null ? data : '';
            }
    ...
    const crud = new GTP_TipoDocumentoFornecedorCrud({
        controllerName: 'GTP_TipoDocumentoFornecedor',
        apiRoute: '/api/gestaoterceirosprestadores/gtp_tipodocumentofornecedor',
        entityName: 'Tipo de Documento de Fornecedor',
        entityNamePlural: 'Tipo de Documento de Fornecedors',
        idField: 'id',
        ...
        exportConfig: {
            ...
            csv: true,
            print: true,
            filename: 'GTP_TipoDocumentoFornecedor'
        }
    });
    ...
    $(document).on('change', '.toggle-ativo', function () {
        ...
        console.log(`🔄 [GTP_TipoDocumentoFornecedor] Toggle Ativo - ID: ${id}, Novo valor: ${newValue}`);
        ...
        toggleDebounceTimer = setTimeout(function () {
            $.ajax({
                url: `/GTP_TipoDocumentoFornecedor/ToggleAtivo`,
                type: 'POST',
                headers: {
                    'RequestVerificationToken': $('input[name="__RequestVerificationToken"]').val()
                },
                ...
                success: function (response) {
                    if (response.success) {
                        console.log(`✅ [GTP_TipoDocumentoFornecedor] Toggle Ativo atualizado - ID: ${id}`);
                        ...
                        console.error(`❌ [GTP_TipoDocumentoFornecedor] Erro ao atualizar Toggle Ativo:`, response);
                ...
                error: function (xhr) {
                    // Reverte toggle em caso de erro
                    $toggle.prop('checked', currentValue);
                    console.error(`❌ [GTP_TipoDocumentoFornecedor] Erro AJAX Toggle Ativo:`, xhr);
    ...
    console.log('✅ [GTP_TipoDocumentoFornecedor] JavaScript inicializado com sucesso!');
});
