/**
 * ============================================================================
 * CAPHISTORICOBLOQUEIOS - JavaScript com Checkbox e Toggle Ativo
 * ============================================================================
 * Arquivo: wwwroot/js/controleacessoportaria/caphistoricobloqueios/caphistoricobloqueios.js
 ...
 * 
 * Implementação específica do CRUD de CapHistoricoBloqueios.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class CapHistoricoBloqueiosCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [CapHistoricoBloqueios] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [CapHistoricoBloqueios] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [CapHistoricoBloqueios] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.DataBloqueio = (formData.dataBloqueio || formData.DataBloqueio) || null;
        ...
        console.log('📤 [CapHistoricoBloqueios] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [CapHistoricoBloqueios] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [CapHistoricoBloqueios] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [CapHistoricoBloqueios] ID vazio para row:', row);
        }
    ...
    const crud = new CapHistoricoBloqueiosCrud({
        controllerName: 'CapHistoricoBloqueios',
        entityName: 'CapHistoricoBloqueios',
        entityNamePlural: 'CapHistoricoBloqueioss',
        idField: 'id',
        ...
        exportConfig: {
            ...
            csv: true,
            print: true,
            filename: 'CapHistoricoBloqueios'
        }
    });
    ...
    console.log('✅ [CapHistoricoBloqueios] JavaScript inicializado com sucesso!');
});
