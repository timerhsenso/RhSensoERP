/**
 * ============================================================================
 * CAPCONTRATOSFORNECEDOR - JavaScript com Checkbox e Toggle Ativo
 * ============================================================================
 * Arquivo: wwwroot/js/controleacessoportaria/capcontratosfornecedor/capcontratosfornecedor.js
 ...
 * 
 * Implementação específica do CRUD de CapContratosFornecedor.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class CapContratosFornecedorCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [CapContratosFornecedor] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [CapContratosFornecedor] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [CapContratosFornecedor] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.NumeroContrato = formData.numeroContrato || formData.NumeroContrato || '';
        ...
        if (formData.valor !== undefined && formData.valor !== null && formData.valor !== '') {
            cleanData.Valor = parseFloat((formData.valor || '0').toString().replace(',', '.'));
        } else {
            cleanData.Valor = null;
        }
        ...
        console.log('📤 [CapContratosFornecedor] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [CapContratosFornecedor] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [CapContratosFornecedor] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [CapContratosFornecedor] ID vazio para row:', row);
        }
    ...
    const crud = new CapContratosFornecedorCrud({
        controllerName: 'CapContratosFornecedor',
        entityName: 'CapContratosFornecedor',
        entityNamePlural: 'CapContratosFornecedors',
        idField: 'id',
        ...
        exportConfig: {
            ...
            csv: true,
            print: true,
            filename: 'CapContratosFornecedor'
        }
    });
    ...
    $(document).on('change', '.toggle-ativo', function () {
        ...
        console.log(`🔄 [CapContratosFornecedor] Toggle Ativo - ID: ${id}, Novo valor: ${newValue}`);
        ...
        toggleDebounceTimer = setTimeout(function () {
            $.ajax({
                url: `/CapContratosFornecedor/ToggleAtivo`,
                type: 'POST',
                headers: {
                ...
                success: function (response) {
                    if (response.success) {
                        console.log(`✅ [CapContratosFornecedor] Toggle Ativo atualizado - ID: ${id}`);
                        ...
                        console.error(`❌ [CapContratosFornecedor] Erro ao atualizar Toggle Ativo:`, response);
                ...
                error: function (xhr) {
                    // Reverte toggle em caso de erro
                    $toggle.prop('checked', currentValue);
                    console.error(`❌ [CapContratosFornecedor] Erro AJAX Toggle Ativo:`, xhr);
    ...
    console.log('✅ [CapContratosFornecedor] JavaScript inicializado com sucesso!');
});
