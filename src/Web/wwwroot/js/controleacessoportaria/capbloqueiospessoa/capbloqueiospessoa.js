/**
 * ============================================================================
 * CAPBLOQUEIOSPESSOA - JavaScript com Checkbox e Toggle Ativo
 * ============================================================================
 * Arquivo: wwwroot/js/controleacessoportaria/capbloqueiospessoa/capbloqueiospessoa.js
 ...
 * 
 * Implementação específica do CRUD de CapBloqueiosPessoa.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class CapBloqueiosPessoaCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [CapBloqueiosPessoa] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [CapBloqueiosPessoa] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [CapBloqueiosPessoa] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.Motivo = formData.motivo || formData.Motivo || '';
        ...
        if (formData.idVisitante !== undefined && formData.idVisitante !== null && formData.idVisitante !== '') {
            const val = parseInt(formData.idVisitante, 10);
            cleanData.IdVisitante = isNaN(val) ? null : val;
        } else if (formData.IdVisitante !== undefined && formData.IdVisitante !== null && formData.IdVisitante !== '') {
            const val = parseInt(formData.IdVisitante, 10);
            cleanData.IdVisitante = isNaN(val) ? null : val;
        } else {
            cleanData.IdVisitante = null;
        }
        ...
        cleanData.DataBloqueio = formData.dataBloqueio || formData.DataBloqueio || new Date().toISOString();

        cleanData.DataDesbloqueio = (formData.dataDesbloqueio || formData.DataDesbloqueio) || null;

        // Guid nullable fields - PascalCase
        cleanData.UsuarioBloqueio = (formData.usuarioBloqueio || formData.UsuarioBloqueio) || null;

        cleanData.UsuarioDesbloqueio = (formData.usuarioDesbloqueio || formData.UsuarioDesbloqueio) || null;


        console.log('📤 [CapBloqueiosPessoa] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [CapBloqueiosPessoa] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [CapBloqueiosPessoa] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [CapBloqueiosPessoa] ID vazio para row:', row);
        }
    ...
    const columns = [
        ...
        {
            data: 'idColaboradorFornecedor',
            name: 'IdColaboradorFornecedor',
            title: 'ID Colaborador Fornecedor',
            orderable: true,
            render: function (data, type, row) {
    ...
    const crud = new CapBloqueiosPessoaCrud({
        controllerName: 'CapBloqueiosPessoa',
        entityName: 'CapBloqueiosPessoa',
        entityNamePlural: 'CapBloqueiosPessoas',
        idField: 'id',
    ...
    $(document).on('change', '.toggle-ativo', function () {
        ...
        console.log(`🔄 [CapBloqueiosPessoa] Toggle Ativo - ID: ${id}, Novo valor: ${newValue}`);
        ...
        toggleDebounceTimer = setTimeout(function () {
            $.ajax({
                url: `/CapBloqueiosPessoa/ToggleAtivo`,
                type: 'POST',
                headers: {
                ...
                success: function (response) {
                    if (response.success) {
                        console.log(`✅ [CapBloqueiosPessoa] Toggle Ativo atualizado - ID: ${id}`);
                        ...
                        console.error(`❌ [CapBloqueiosPessoa] Erro ao atualizar Toggle Ativo:`, response);
                ...
                error: function (xhr) {
                    // Reverte toggle em caso de erro
                    $toggle.prop('checked', currentValue);
                    console.error(`❌ [CapBloqueiosPessoa] Erro AJAX Toggle Ativo:`, xhr);
    ...
    console.log('✅ [CapBloqueiosPessoa] JavaScript inicializado com sucesso!');
});
