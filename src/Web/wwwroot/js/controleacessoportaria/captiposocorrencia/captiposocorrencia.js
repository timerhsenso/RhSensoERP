/**
 * ============================================================================
 * CADASTRO DE TIPO DE OCORRÊNCIA - JavaScript com Checkbox e Toggle Ativo
 * ============================================================================
 * Arquivo: wwwroot/js/controleacessoportaria/captiposocorrencia/captiposocorrencia.js
 ...
 * 
 * Implementação específica do CRUD de Cadastro de Tipo de Ocorrência.
 * Estende a classe CrudBase com customizações necessárias.
 * ============================================================================
 */

class CapTiposOcorrenciaCrud extends CrudBase {
    constructor(config) {
        super(config);
    ...
    enablePrimaryKeyFields(enable) {
        ...
        if (enable) {
            ...
            console.log('✏️ [CapTiposOcorrencia] Campo PK habilitado para edição (criação)');
        } else {
            // Edição: campo readonly
            $pkField.prop('readonly', true)
                    .addClass('bg-light');
            console.log('🔒 [CapTiposOcorrencia] Campo PK desabilitado (edição)');
        }
    }
    ...
    beforeSubmit(formData, isEdit) {
        console.log('📥 [CapTiposOcorrencia] Dados ANTES:', JSON.parse(JSON.stringify(formData)));
        ...
        cleanData.Severidade = formData.severidade || formData.Severidade || '';


        // Boolean fields - PascalCase
        cleanData.Ativo = formData.ativo === true || formData.Ativo === 'true' || false;

        console.log('📤 [CapTiposOcorrencia] Dados DEPOIS (PascalCase):', JSON.parse(JSON.stringify(cleanData)));
        return cleanData;
    }
    ...
    afterSubmit(data, isEdit) {
        console.log('✅ [CapTiposOcorrencia] Registro salvo:', data);
...
$(document).ready(function () {
    ...
    console.log('🔐 [CapTiposOcorrencia] Permissões ativas:', window.crudPermissions);
    ...
    function getCleanId(row, fieldName) {
        ...
        // Log para debug
        if (!id) {
            console.warn('⚠️ [CapTiposOcorrencia] ID vazio para row:', row);
        }
    ...
    const crud = new CapTiposOcorrenciaCrud({
        controllerName: 'CapTiposOcorrencia',
        apiRoute: '/api/gestaoterceirosprestadores/captiposocorrencia',
        entityName: 'Cadastro de Tipo de Ocorrência',
        entityNamePlural: 'Cadastro de Tipo de Ocorrências',
        idField: 'id',
    ...
    $(document).on('change', '.toggle-ativo', function () {
        ...
        console.log(`🔄 [CapTiposOcorrencia] Toggle Ativo - ID: ${id}, Novo valor: ${newValue}`);
        ...
        toggleDebounceTimer = setTimeout(function () {
            $.ajax({
                url: `/CapTiposOcorrencia/ToggleAtivo`,
                type: 'POST',
                headers: {
                    'RequestVerificationToken': $('input[name="__RequestVerificationToken"]').val()
                },
                ...
                success: function (response) {
                    if (response.success) {
                        console.log(`✅ [CapTiposOcorrencia] Toggle Ativo atualizado - ID: ${id}`);
                        ...
                        console.error(`❌ [CapTiposOcorrencia] Erro ao atualizar Toggle Ativo:`, response);
                ...
                error: function (xhr) {
                    // Reverte toggle em caso de erro
                    $toggle.prop('checked', currentValue);
                    console.error(`❌ [CapTiposOcorrencia] Erro AJAX Toggle Ativo:`, xhr);
    ...
    console.log('✅ [CapTiposOcorrencia] JavaScript inicializado com sucesso!');
});
