/**
 * ============================================================================
 * CADASTRO DE TIPO DE TREINAMENTO - JavaScript com Ordenação de Navegações
 * ============================================================================
 * Arquivo: wwwroot/js/treinamentodesenvolvimento/tretipostreinamento/tretipostreinamento.js
 ...
 * 
 * Implementação específica do CRUD de Cadastro de Tipo de Treinamento.
...
class TreTiposTreinamentoCrud extends CrudBase {
    ...
    beforeSubmit(formData, isEdit) {
        ...
        cleanData.DiasPrazoValidade = parseInt(formData.diasPrazoValidade || formData.DiasPrazoValidade || 0, 10);
        cleanData.CargaHoraria = parseInt(formData.cargaHoraria || formData.CargaHoraria || 0, 10);
...
$(document).ready(function () {
    ...
    const crud = new TreTiposTreinamentoCrud({
        controllerName: 'TreTiposTreinamento',
        apiRoute: '/api/treinamentodesenvolvimento/tretipostreinamento',
        entityName: 'Cadastro de Tipo de Treinamento',
        entityNamePlural: 'Cadastro de Tipo de Treinamentos',
        idField: 'id',
    ...
    $(document).on('change', '.toggle-ativo', function () {
        ...
        console.log(`🔄 [TreTiposTreinamento] Toggle Ativo - ID: ${id}, Novo valor: ${newValue}`);
        ...
        toggleDebounceTimer = setTimeout(function () {
            $.ajax({
                url: `/TreTiposTreinamento/ToggleAtivo`,
                type: 'POST',
                headers: {
                    'RequestVerificationToken': $('input[name="__RequestVerificationToken"]').val()
                },
                ...
                success: function (response) {
                    if (response.success) {
                        console.log(`✅ [TreTiposTreinamento] Toggle Ativo atualizado - ID: ${id}`);
                        ...
                        console.error(`❌ [TreTiposTreinamento] Erro ao atualizar Toggle Ativo:`, response);
                ...
                error: function (xhr) {
                    // Reverte toggle em caso de erro
                    $toggle.prop('checked', currentValue);
                    console.error(`❌ [TreTiposTreinamento] Erro AJAX Toggle Ativo:`, xhr);
    ...
    console.log('✅ [TreTiposTreinamento] JavaScript inicializado com sucesso!');
});
