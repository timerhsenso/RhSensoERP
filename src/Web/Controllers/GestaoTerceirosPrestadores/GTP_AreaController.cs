...
using RhSensoERP.Web.Services.GestaoTerceirosPrestadores.GTP_Area;
...
[MenuItem(
    ...
    DisplayName = "Cadastro de Terceiros teste",
    Icon = "fas fa-table",
    Order = 10,
...
/// <summary>
/// Controller para gerenciamento de Cadastro de Terceiros teste.
/// Herda toda a funcionalidade CRUD de BaseCrudController.
/// </summary>
[Authorize]
public class GTP_AreaController 
    : BaseCrudController<GTP_AreaDto, CreateGTP_AreaRequest, UpdateGTP_AreaRequest, int>
{
    // =========================================================================
    ...
    /// </summary>
    private const string CdSistema = "GTP";

    private readonly IGTP_AreaApiService _gtp_areaService;

    // =========================================================================
    // CONSTRUTOR
    // =========================================================================

    public GTP_AreaController(
        IGTP_AreaApiService apiService,
        IUserPermissionsCacheService permissionsCache,
        ILogger<GTP_AreaController> logger)
        : base(apiService, permissionsCache, logger)
    {
        _gtp_areaService = apiService;
    }
    ...
    public async Task<IActionResult> Index(CancellationToken ct = default)
    {
        ...
        var viewModel = new GTP_AreaListViewModel
        {
        ...
        return View("~/Views/GestaoTerceirosPrestadores/GTP_Area/Index.cshtml", viewModel);
    }
    ...
    public override async Task<IActionResult> Create([FromBody] CreateGTP_AreaRequest dto)
    {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit([FromQuery] int id, [FromBody] UpdateGTP_AreaRequest dto)
    {
    ...
    [HttpPut]
    [ValidateAntiForgeryToken]
    public override async Task<IActionResult> Update(int id, [FromBody] UpdateGTP_AreaRequest dto)
    {
        if (!await CanEditAsync(CdFuncao))
        {
            return JsonError("Você não tem permissão para alterar registros nesta tela.");
    ...
    public async Task<IActionResult> ToggleAtivo([FromBody] ToggleAtivoRequest request, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            await _gtp_areaService.ToggleAtivoAsync(request.Id, request.Ativo, ct);
