...
// Entity: BotaoFuncao
// Module: Seguranca
// ApiRoute: api/seguranca/botaofuncao
...
/// <summary>
/// Serviço de API para Botões de Função.
...
/// </summary>
public class BotaoFuncaoApiService 
    : BaseApiService<BotaoFuncaoDto, CreateBotaoFuncaoRequest, UpdateBotaoFuncaoRequest, string>,
      IBotaoFuncaoApiService
{
    private const string ApiRoute = "api/seguranca/botaofuncao";
    ...
    // CONSTRUTOR
    // =========================================================================
    public BotaoFuncaoApiService(
        HttpClient httpClient,
        ILogger<BotaoFuncaoApiService> logger,
    ...
    /// </summary>
    public override async Task<ApiResponse<BotaoFuncaoDto>> GetByIdAsync(string id)
    {
        try
        {
            ...
            _logger.LogDebug("🔍 [BOTAOFUNCAO] GetByIdAsync - Key: {Id} → URL: {Url}", id, url);
            ...
            {
                var result = JsonSerializer.Deserialize<ApiResponse<BotaoFuncaoDto>>(content, _jsonOptions);
                return result ?? new ApiResponse<BotaoFuncaoDto>
                {
                    Success = false,
            ...
            _logger.LogWarning("⚠️ [BOTAOFUNCAO] GetByIdAsync - ID: {Id}, Status: {Status}, Body: {Body}",
                id, response.StatusCode, content);
            return new ApiResponse<BotaoFuncaoDto>
            {
                Success = false,
        ...
        {
            _logger.LogError(ex, "❌ [BOTAOFUNCAO] Erro GetByIdAsync {Id}", id);
            return new ApiResponse<BotaoFuncaoDto>
            {
                Success = false,
                Error = new ApiError { Message = "Erro ao buscar registro" }
    ...
    /// </summary>
    public override async Task<ApiResponse<BotaoFuncaoDto>> UpdateAsync(string id, UpdateBotaoFuncaoRequest dto)
    {
        try
        {
            await AddAuthorizationHeaderAsync();
            ...
            _logger.LogWarning("✏️ [BOTAOFUNCAO] UpdateAsync - ID: {Id} → URL: {Url} | Body: {Body}", id, url, json);

            var response = await _httpClient.PutAsync(url, httpContent);
            var content = await response.Content.ReadAsStringAsync();

            // ⭐ v6.3: Loga response raw
            _logger.LogDebug("✏️ [BOTAOFUNCAO] UpdateAsync - Status: {Status} | Response: {Body}",
            ...
            _logger.LogWarning("⚠️ [BOTAOFUNCAO] UpdateAsync FALHOU - ID: {Id}, Status: {Status}, Response: {Body}",
                id, response.StatusCode, content);
        ...
        {
            _logger.LogError(ex, "❌ [BOTAOFUNCAO] Erro UpdateAsync {Id}", id);
            return new ApiResponse<BotaoFuncaoDto>
            {
                Success = false,
    ...
    public override async Task<ApiResponse<bool>> DeleteAsync(string id)
    {
        try
        {
            ...
            var path = CompositeKeyToPath(id);
            var url = $"{_baseEndpoint}/{path}";
            _logger.LogDebug("🗑️ [BOTAOFUNCAO] DeleteAsync - Key: {Id} → URL: {Url}", id, url);
            ...
            if (response.IsSuccessStatusCode)
            {
                _logger.LogInformation("✅ [BOTAOFUNCAO] Registro {Id} excluído", id);
                return new ApiResponse<bool> { Success = true, Data = true };
            }

            _logger.LogWarning("⚠️ [BOTAOFUNCAO] DeleteAsync - ID: {Id}, Status: {Status}, Body: {Body}",
                id, response.StatusCode, content);
        ...
        {
            _logger.LogError(ex, "❌ [BOTAOFUNCAO] Erro DeleteAsync {Id}", id);
            return new ApiResponse<bool>
            {
                Success = false,
    ...
    public async Task<ApiResponse<BatchDeleteResultDto>> DeleteBatchAsync(IEnumerable<string> ids)
    {
        try
        {
            ...
            _logger.LogDebug("[BOTAOFUNCAO] DELETE {Route}/batch - Body: {Body}", _baseEndpoint, json);
        ...
        {
            _logger.LogError(ex, "[BOTAOFUNCAO] Erro de conexão em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
        ...
        {
            _logger.LogError(ex, "[BOTAOFUNCAO] Exceção em DeleteBatchAsync");
            return new ApiResponse<BatchDeleteResultDto>
            {
                Success = false,
