...
const SchemaValidator = {
  ...
  "columns": [
    ...
    compare() {
        ...
        // Mapeia colunas do banco por nome normalizado
        const dbColumnsMap = new Map();
        dbSchema.columns.forEach(col => {
            dbColumnsMap.set(this.normalizeName(col.name), col);
        });
        // Mapeia propriedades da entidade por nome normalizado
        const entityPropsMap = new Map();
        entity.properties.forEach(prop => {
            entityPropsMap.set(this.normalizeName(prop.name), prop);
        });
        const results = [];
        let okCount = 0;
        let warningCount = 0;
        ...
        entity.properties.forEach(prop => {
            // Tenta encontrar pela chave normalizada
            const dbCol = dbColumnsMap.get(this.normalizeName(prop.name));
        ...
        const extraColumns = [];
        dbSchema.columns.forEach(col => {
            if (!entityPropsMap.has(this.normalizeName(col.name))) {
    ...
    // Normaliza nome para comparação (remove _, acentos e casing)
    normalizeName(name) {
        if (!name) return '';
        return name.toLowerCase()
            .replace(/_/g, '') // Remove underscores (snake_case -> pascalcase/camelcase)
    ...
    columnToProperty(column) {
        // Converte snake_case para PascalCase se necessário
        let propName = column.name;
        // Se tem underscore, converte para PascalCase
        if (propName.includes('_')) {
            propName = propName.toLowerCase()
                .split('_')
                .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                .join('');
            // Remove acentos do nome da propriedade
            propName = propName.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        }
        return {
            name: propName, // Antes: column.name
