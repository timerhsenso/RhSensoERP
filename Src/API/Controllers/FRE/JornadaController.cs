...
[HttpGet]
    public async Task<IActionResult> List([FromQuery] int page = 1, [FromQuery] int pageSize = 50, [FromQuery] int? cdEmpresa = null, [FromQuery] int? cdFilial = null, [FromQuery] string? cdJornada = null)
    {
        var query = _db.Set<Jornada>().AsNoTracking().AsQueryable();
        ...
        if (!string.IsNullOrWhiteSpace(cdJornada)) query = query.Where(x => x.CdJornada == cdJornada);
        ...
        var items = await query
            ...
            .Take(pageSize)
            .Select(x => new JornadaListItemDto { })
            .ToListAsync();
        return Ok(new { total, page, pageSize, items });
    }

[HttpGet("find")]
    public async Task<IActionResult> Find([FromQuery] int? cdEmpresa = null, [FromQuery] int? cdFilial = null, [FromQuery] string? cdJornada = null)
    {
        var q = _db.Set<Jornada>().AsNoTracking().AsQueryable();
        if (cdEmpresa.HasValue) q = q.Where(x => x.CdEmpresa == cdEmpresa.Value);
        if (cdFilial.HasValue) q = q.Where(x => x.CdFilial == cdFilial.Value);
        if (!string.IsNullOrWhiteSpace(cdJornada)) q = q.Where(x => x.CdJornada == cdJornada);
